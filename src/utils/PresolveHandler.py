...
def presolve_papilo(model: Model, method: PresolvingMethod = PresolvingMethod.CoeffTightening, **kwargs) -> Model:
    """
    ...
    cmd_presolve = [
        str(PAPILO_PATH),
        'presolve',
        '-f', str(model.model_path.absolute()),
        '-r', str(model.reduced_path.absolute()),
        '-o', str(model.postsolve_path.absolute())
    ]
    
    try:
        print(f"Running PaPILO presolve: {' '.join(cmd_presolve)}")
        result = subprocess.run(cmd_presolve, check=True, capture_output=True, text=True)
        print(result.stdout)
    ...
    def presolve(
        ...
        **kwargs
        ...
        match presolver:
            ...
            case _:
                raise NotImplementedError(f"Presolver {presolver} not implemented")
