# MPS Input File = models/formats/mps/10teams/10teams.mps
...
stagei = gp.Parameter(cont,
                      domain=[i],
                      description="block numbers of equations in decomposition")
stages = gp.Parameter(cont,
                      domain=[s],
                      description="block numbers of SOS in decomposition")
ac = gp.Parameter(cont,
                  domain=[i, jc],
                  description="matrix coefficients: continuous variables")
ab = gp.Parameter(cont,
                  domain=[i, jb],
                  description="matrix coefficients: binary variables")
ai = gp.Parameter(cont,
                  domain=[i, ji],
                  description="matrix coefficients: integer variables")
asc = gp.Parameter(cont,
                   domain=[i, jsc],
                   description="matrix coefficients: semi-continuous variables")
asi = gp.Parameter(cont,
                   domain=[i, jsi],
                   description="matrix coefficients: semi-integer variables")
as1 = gp.Parameter(cont,
                   domain=[i, s, j],
                   description="matrix coefficients: sos 1 variables")
as2 = gp.Parameter(cont,
                   domain=[i, s, j],
                   description="matrix coefficients: sos 2 variables")

qobj = gp.Parameter(cont,
                    domain=[v, j, v, j],
                    description="Q term for objective")
q = gp.Parameter(cont,
                 domain=[e, i, v, j, v, j],
                 description="Q terms by equation type")
...
r = gp.Variable(cont,
                type="positive",
                domain=[i],
                description="ranged row variables")
xb = gp.Variable(cont,
                 type="binary",
                 domain=[j],
...
xsc = gp.Variable(cont,
                  type="semicont",
                  domain=[j],
                  description="semi-continuous variables")
xsi = gp.Variable(cont,
                  type="semiint",
                  domain=[j],
                  description="semi-integer variables")
xs1 = gp.Variable(cont,
                  type="sos1",
                  domain=[s, j],
                  description="SOS 1 variables")
xs2 = gp.Variable(cont,
                  type="sos2",
                  domain=[s, j],
                  description="SOS 2 variables")

cont.loadRecordsFromGdx('models/formats/gdx/10teams/10teams.gdx')
...
# mps_model.convert('models/formats/mps/10teams_backtomps',
